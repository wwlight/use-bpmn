...
const mergedHeaderClass = `relative px-20 py-16 ${headerClass ?? ''}`.trim()
const mergedBodyClass = `flex flex-col of-hidden ${!slots.body ? 'p-0' : 'px-20'} ${bodyClass ?? ''}`.trim()
...
<template>
  <el-dialog
    v-model="innerVisible" destroy-on-close align-center :width="width < 1500 ? '60%' : '50%'"
    class="grid grid-rows-[auto_1fr_auto] grid-cols-1 max-h-80% min-h-400 min-w-500 of-hidden [&_.el-dialog\_\_headerbtn]:(text-20 flex-center right-5 top-5) !p-0"
